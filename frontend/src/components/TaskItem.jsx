...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  const menuRef = useRef(null)  // <--- ref for menu dropdown container
    ...
    <>
      <div className={`${TI_CLASSES.wrapper} ${borderColor}`}>        
        <div className={TI_CLASSES.leftContainer}>
          {showCompleteCheckbox && (
            <button
              ...
              className={`${TI_CLASSES.completeBtn} ${isCompleted ? 'text-green-500' : 'text-gray-300'} cursor-pointer`}
            >
              <CheckCircle2
                size={18}
                className={`${TI_CLASSES.checkboxIconBase} ${isCompleted ? 'fill-white' : ''}`}
              />
            </button>
          )}
        ...
        <div className={TI_CLASSES.rightContainer}>
          <div className="relative" ref={menuRef}> {/* <-- Attach ref here */}
            <button onClick={() => setShowMenu(!showMenu)} className={`${TI_CLASSES.menuButton} cursor-pointer`}>
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map(opt => (
                  <button
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-purple-100 flex items-center gap-2 transition-colors duration-200 cursor-pointer"
                  >
                    {opt.icon}{opt.label}
                  </button>
                ))}
